...
class DynamicsVisualizer:
    ...
    def reset(self, train_gs=True):
        pcd, imgs, masks = self.pm.get_tabletop_points_env(self.env, obj_names=['keyboard'], return_imgs=True)
        self.imgs = imgs
        self.masks = masks
    ...
    def step_sim(self, action):  # (2, 3)
        ...
        rendervar_list, visvar_list = self.gs_trainer.rollout_and_render(self.dm, self.env, action, vis_dir=vis_dir_rollout)
